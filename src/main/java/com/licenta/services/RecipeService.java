...
public class RecipeService {
    ...
    public RecipeEatenDigest getRecipeEatenDigest(Long id) {
        ...
        List<RecipeEaten> recipeEatenList = new ArrayList<>();
        recipeEatenList.add(recipeEaten);
        ...
        return new RecipeEatenDigest(recipeEatenList, digestList, recipeEaten.getRecipe().getCalories() * recipeEaten.getRatio(), recipeEaten.getRecipe().getTotalWeight() * recipeEaten.getRatio());
    }
    ...
    public RecipeEatenDigest getDigestFromDate(User user, LocalDate localDate) {
        ...
        if (recipes.size() != 0) {
            ...
            recipeEatenList.add(recipes.get(0));
            ...
            if (recipes.size() > 1) {
                for (int i = 1; i < recipes.size(); i++) {
                    ...
                    recipeEatenList.add(recipes.get(i));
            ...
            totalWeight = 0;
            for (int i = 0; i < 26; i++) {
                Digest digest = new Digest(0, 0);
                recipesDigest.add(digest);
            }
        }

        return new RecipeEatenDigest(recipeEatenList, recipesDigest, calories, totalWeight);
    }
}
