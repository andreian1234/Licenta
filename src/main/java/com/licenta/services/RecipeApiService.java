    ...
    public static RootRecipeDTO getRootRecipeDetailsObject(JsonObject jsonObject) throws Exception {
        ...
        for (int i = 0; i < recipesJsonArray.size(); i++) {
            ...
            for (int j = 0; j < ingredientsJsonArray.size(); j++) {
                ...
                } else ingredientDTOObj.setWeight(((JsonObject) ingredientsJsonArray.get(j)).get("weight").getAsDouble());
                ...
                } else ingredientDTOObj.setFoodId(((JsonObject) ingredientsJsonArray.get(j)).get("foodId").getAsString());
